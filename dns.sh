...
log_settings_menu() {
  while true; do
    ...
    echo "0. 返回上一级"
    echo "=================================================="
    read -r -p "请选择操作: " subopt
    echo

    case "${subopt}" in
      1) set_mosdns_log_level_menu ;;
      2) show_recent_logs_50 ;;
      3) follow_mosdns_logs ;;
      0) return ;;
      *) c_warn "无效选项" ;;
    esac

    echo
    read -r -p "按回车继续返回日志菜单..." _
    echo
  done
}
