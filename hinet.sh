...
validate_config() {
    local bad=0
    [[ -n "${SHOW_IP_API_URL:-}" ]] || { wlog "❌ 配置缺失：SHOW_IP_API_URL 获取当前 IP API 未配置。"; bad=1; }
    [[ -n "${CHANGE_IP_API_URL:-}" ]] || { wlog "❌ 配置缺失：CHANGE_IP_API_URL 更换 IP API 未配置。"; bad=1; }
    [[ -n "${CHECK_TARGET:-}" ]] || { wlog "❌ 配置缺失：CHECK_TARGET 检测目标未配置。"; bad=1; }
    return "$bad"
}
is_public_ipv4() {
    local ip="$1" a b c d IFS=.
...
get_current_ip_from_api() {
    ...
    wlog "❌ 获取当前 IP API 未返回公网 IPv4。返回摘要：$(shorten "$body")"
    return 1
}
append_history() {
    local old_ip="$1" new_ip="$2" old_ddns="$3" new_ddns="$4" reason="$5" note="$6"
    mkdirs
    printf '%s\told_ip=%s\tnew_ip=%s\told_ddns=%s\tnew_ddns=%s\treason=%s\tnote=%s\n' \
        "$(now_human)" "$old_ip" "$new_ip" "$old_ddns" "$new_ddns" "$reason" "$note" >> "$HISTORY_FILE" 2>/dev/null || true
}

globalping_create_measurement() {
    local target="$1" payload body id
    payload="$(jq -nc --arg target "$target" --arg country "CN" --argjson limit "${CN_PROBES:-2}" --argjson packets "${GP_PACKETS:-3}" \
        '{type:"ping",target:$target,locations:[{country:$country,limit:$limit}],measurementOptions:{packets:$packets}}' 2>/dev/null)"
    [[ -n "$payload" ]] || { wlog "⚠️ 生成 Globalping 请求 JSON 失败。"; return 1; }
    body="$(curl -sS -L --connect-timeout 10 --max-time "$CURL_TIMEOUT" -H 'Content-Type: application/json' -d "$payload" "${GLOBALPING_API_BASE}/measurements" 2>&1)"
    id="$(printf '%s' "$body" | jq -r '.id // .measurementId // empty' 2>/dev/null)"
    if [[ -z "$id" || "$id" == "null" ]]; then
        wlog "⚠️ Globalping 创建 measurement 失败，不计入连续失败。返回摘要：$(shorten "$body")"
        return 1
    fi
    printf '%s' "$id"
}
...
globalping_parse_result() {
    jq -r '
      def n: (tonumber? // 0);
      def raw: (.result.rawOutput? // .rawOutput? // "");
      def rcv: ((.result.stats.rcv? // .result.stats.received? // .result.stats.packetsReceived? // .result.stats.receivedPackets? // .result.stats.packets_received? // 0) | n);
      def loss: ((.result.stats.loss? // .result.stats.packetLoss? // .result.stats.packet_loss? // 100) | tonumber? // 100);
      def ok_probe: ((rcv > 0) or (loss < 100) or ((raw | test("(?i)(bytes from|ttl=|time=[0-9]+[.]?[0-9]*[[:space:]]*ms)"))));
      [.results[]?] as $r |
      ($r | length) as $total |
      ($r | map(select(ok_probe)) | length) as $ok |
      (.status // "unknown") as $status |
      [$status, $total, $ok] | @tsv
    ' 2>/dev/null
}
...
globalping_check_target() {
    ...
    while true; do
        ...
        if [[ "$total" =~ ^[0-9]+$ && "$okn" =~ ^[0-9]+$ && "$total" -gt 0 ]]; then
            case "$status" in
                finished|done|completed)
                    if (( okn > 0 )); then
                        wlog "✅ Globalping CN ping 正常：target=${target}，ok=${okn}/${total}，measurement=${id}"
                        return 0
                    fi
                    wlog "❌ Globalping CN ping 全部失败：target=${target}，ok=0/${total}，measurement=${id}"
                    return 1
                    ;;
                *) ;;
            esac
        ...
        if (( $(now_epoch) >= deadline )); then
            ...
            wlog "⚠️ Globalping 等待结果超时或无中国探针结果，measurement=${id}，不计入连续失败。最后状态=${status:-unknown}"
            return 2
        fi
        sleep 3
    done
...
change_ip_worker() {
    local reason="${1:-timer_auto}" official="1"
    mkdirs
    exec 8>"$CHANGE_LOCK_FILE"
    if ! flock -n 8; then
        wlog "⚠️ 更换 IP 锁被占用，已有换 IP 流程正在执行，本次跳过。reason=${reason}"
        return 1
    fi
    load_status
    local now last_delta old_ip old_ddns body returned_ip new_ip new_ddns note
    ...
    last_delta=$(( now - ${LAST_API_CALL_EPOCH:-0} ))
    if (( MIN_API_INTERVAL > 0 && LAST_API_CALL_EPOCH > 0 && last_delta < MIN_API_INTERVAL )); then
        wlog "⏳ 距离上次调用更换 IP API 仅 ${last_delta}s，小于最小间隔 ${MIN_API_INTERVAL}s，本次不调用。"
        return 1
    fi
    old_ip="$(get_current_ip_from_api 2>/dev/null || true)"
    old_ddns="$(resolve_target_ip 2>/dev/null || true)"
    ...
    LAST_API_CALL_EPOCH="$(now_epoch)"
    save_status
    wlog "🧾 更换 IP API 返回摘要：$(shorten "$body")"
    returned_ip="$(extract_public_ipv4 "$body" 2>/dev/null || true)"
    [[ -n "$returned_ip" ]] && wlog "ℹ️ 更换 IP API 返回公网 IP：${returned_ip}"
    if (( POST_CHANGE_WAIT_SECONDS > 0 )); then
    ...
    new_ddns="$(resolve_target_ip 2>/dev/null || true)"
    note="api_called"
    [[ -n "$returned_ip" ]] && note="api_returned_public_ip"
    if [[ -n "$old_ip" && -n "$new_ip" && "$old_ip" == "$new_ip" && -n "$old_ddns" && -n "$new_ddns" && "$old_ddns" == "$new_ddns" ]]; then
        wlog "⚠️ API 已调用，但当前 IP/DDNS 暂未变化：old_ip=${old_ip}，new_ip=${new_ip}，old_ddns=${old_ddns}，new_ddns=${new_ddns}。可能是 API 错误、DDNS 未更新或商家暂未完成切换。"
        note="api_called_but_ip_not_changed"
    fi
    wlog "✅ 换 IP 流程完成：${old_ip:-unknown} -> ${new_ip:-${returned_ip:-unknown}}，DDNS：${old_ddns:-unknown} -> ${new_ddns:-unknown}，reason=${reason}，note=${note}"
    LAST_CHANGE_EPOCH="$(now_epoch)"
    LAST_RESOLVED_IP="$new_ddns"
    save_status
    append_history "${old_ip:-unknown}" "${new_ip:-${returned_ip:-unknown}}" "${old_ddns:-unknown}" "${new_ddns:-unknown}" "$reason" "$note"
...
main_worker() {
    mkdirs
    wlog "🚪 worker entry：开始执行完整定时检测，version=${APP_VERSION}，pid=$$。"
    load_config
    load_status
    if ! validate_config; then
        LAST_RESULT="config_invalid"; LAST_CHECK_EPOCH="$(now_epoch)"; save_status
        wlog "🏁 worker exit：配置无效，本轮跳过。"
        exit 0
    fi
    local resolved_ip rc now last_delta
    resolved_ip="$(resolve_target_ip 2>/dev/null || true)"
    ...
    if [[ "$rc" -eq 0 ]]; then
        ...
        if (( FAILURE_COUNT >= FAIL_THRESHOLD )); then
            ...
            last_delta=$(( now - ${LAST_CHANGE_EPOCH:-0} ))
            if (( COOLDOWN_SECONDS > 0 && LAST_CHANGE_EPOCH > 0 && last_delta < COOLDOWN_SECONDS )); then
                wlog "⏳ 达到失败阈值，但仍在冷却期：剩余 $(( COOLDOWN_SECONDS - last_delta )) 秒，本次不换 IP。"
            else
                wlog "🚨 达到失败阈值，开始自动调用更换 IP API。"
                if change_ip_worker "globalping_cn_ping_failed_${FAILURE_COUNT}_times"; then
                    FAILURE_COUNT=0
                    LAST_RESULT="changed_ip"
                    LAST_RESOLVED_IP="$(resolve_target_ip 2>/dev/null || true)"
                    save_status
                    wlog "✅ 自动换 IP 完成，失败计数已清零。"
                else
                    LAST_RESULT="change_ip_failed"
                    save_status
                    wlog "❌ 自动换 IP 失败，保留失败计数，等待下一轮。"
                fi
            fi
        fi
    else
    ...
    wlog "🏁 worker exit：定时检测结束，last_result=${LAST_RESULT}，failure_count=${FAILURE_COUNT}/${FAIL_THRESHOLD}。"
    exit 0
}
main_worker
EOF_RUNNER
...
doctor() {
    ...
    if [[ -x "$RUNNER_PATH" ]]; then ok "worker 存在且可执行：$RUNNER_PATH"; else warn "worker 不存在或不可执行，执行 init/start 会自动生成。"; fi
